...
export function RecoveryKeyDisplay({ onAcknowledged }: RecoveryKeyDisplayProps) {
  ...
  function handleCopy() {
    ...
    toast.success(t("settings.recoveryKeyCopied"));
  }

  function handleDownload() {
    const blob = new Blob(
      [
        `Ironlox Recovery Key\n\n${key}\n\nSave this key in a secure location. You will need it to recover your account if you lose your master password.`,
      ],
      { type: "text/plain" },
    );
    ...
    toast.success(t("settings.recoveryKeyDownloaded"));
  }
  ...
  return (
    <Card>
      <CardHeader className="text-center">
        ...
        <CardTitle>{t("settings.recoverySetupTitle")}</CardTitle>
        <CardDescription>{t("settings.recoverySetupDesc")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="flex-1" size="sm" onClick={handleCopy}>
            <Copy className="size-3.5 mr-1" /> {t("vault.copy")}
          </Button>
...
export function RecoverFromKey({ onBack }: { onBack: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    toast.error(t("settings.recoveryComingSoon"));
  }
  ...
  return (
    <Card>
      <CardHeader className="text-center">
        ...
        <CardDescription>{t("recovery.enterKeyDesc")}</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="recovery-key">{t("recovery.recoveryKey")}</Label>
            <Input
              id="recovery-key"
              value={key}
